...
def crop_image(loc_c, score_c, indices_c, num_preds, file):
  ...
  print(filename)
  img = Image.open(file)
  img.save(save_path+filename,quality=95)

  for i in range(num_preds.value):
    filename_tmp = filename.replace(ext,'') + '_' + str('{0:02d}'.format(i+1)) +'_'+ext
    index = indices_c[i]
    cx, cy, w, h, angle = loc_c[5*index],loc_c[5*index+1],loc_c[5*index+2],loc_c[5*index+3],loc_c[5*index+4]
